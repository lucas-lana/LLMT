...
if ! command -v python3.13 &> /dev/null; then
    echo "Python 3.13 não está instalado. Instalando..."
    ...
    # Instala o Python 3.13
    sudo apt-get install -y python3.13 python3.13-distutils || erro "Falha ao instalar Python 3.13."
else
    echo "Python 3.13 já está instalado."
fi

# Verifica se o pip está instalado
if ! python3.13 -m pip --version &> /dev/null; then
    echo "Instalando pip..."
    curl -sS https://bootstrap.pypa.io/get-pip.py | python3.13 || erro "Falha ao instalar pip."
fi

# Atualiza o pip
echo "Atualizando pip..."
python3.13 -m pip install --upgrade pip || erro "Falha ao atualizar pip."
...
if [ -f "requirements.txt" ]; then
    ...
    python3.13 -m pip install -r requirements.txt || erro "Falha ao instalar dependências do requirements.txt."
else
